...
export function AccountsReceivable() {
  ...
  return (
    <div className="p-6">
      <div className="mb-6"><h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3"><Wallet className="w-8 h-8 text-blue-600" /> Accounts Receivable</h1><p className="text-gray-500 mt-1">Manage receivables from customers</p></div>
        ...
        <table className="w-full"><thead className="bg-gray-50 border-b border-gray-200"><tr>{['Customer','Ref No.','Due Date','Total','Received','Outstanding','Status','Action'].map(h=><th key={h} className="text-left px-4 py-3 text-sm font-semibold text-gray-900">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-gray-200">{filtered.map(r=>(
            <tr key={r.id} className="hover:bg-gray-50">
              ...
              <td className="px-4 py-4 text-sm"><button onClick={()=>setSelected(r)} className="text-blue-600 hover:text-blue-800">View</button></td>
            </tr>
          ))}
          </tbody>
        </table>
      ...
      <Modal open={!!selected} onClose={()=>setSelected(null)} title="Receivable Details" size="md" footer={<ModalBtn onClick={()=>setSelected(null)}>Close</ModalBtn>}>
